...
import { localizeBook } from "@/lib/useLocalized";
import { fetchResourceBySlug, mediaUrl } from "@/lib/api";
import { whatsappUrl } from "@/lib/site";
import { ArrowLeft, Headphones, MessageCircle, BookOpen } from "lucide-react";
...
export default function ResourceDetail() {
      ...
      <Layout>
        <div className="container py-24 text-center">
          ...
          <Button asChild className="mt-4">
            <Link to="/resources">{t("resourceDetail.backToResources")}</Link>
          </Button>
        </div>
      </Layout>
    );
  ...
  const relatedBooks = books
    ...
    .map((b) => localizeBook(b, lang));

  // Resolve each stored audio URL (/media/audio/...) against the API base.
  const audios = (resource.audios ?? []).map((a) => ({
    ...a,
    src: a.url?.startsWith("/media/") ? mediaUrl(a.url.replace(/^\/media\//, "")) : a.url,
  }));
    ...
    <Layout>
      ...
      <section className="container py-12 max-w-3xl">
        <Button asChild variant="ghost" size="sm" className="mb-6">
          <Link to="/resources">
            <ArrowLeft className="me-1 h-4 w-4" /> {t("resourceDetail.backToResources")}
          </Link>
        </Button>

        <div className="flex gap-2 mb-4 flex-wrap items-center">
          <Badge className={GRADE_BADGE[resource.grade]}>{t(`grades.${resource.grade}`)}</Badge>
          {resource.pageNumber != null && (
            <Badge variant="secondary" className="gap-1">
              <BookOpen className="h-3 w-3" /> Page {resource.pageNumber}
            </Badge>
          )}
        </div>

        <h1 className="font-serif text-2xl sm:text-3xl font-bold md:text-4xl">{title}</h1>
          ...
          <div className="mt-6 rounded-xl border bg-card p-6">
            ...
            {audios.length > 0 ? (
              <div className="space-y-4">
                {audios.map((a, i) => (
                  <div key={a.fileName || i}>
                    {audios.length > 1 && (
                      <p className="text-xs font-medium text-muted-foreground mb-1 truncate">
                        {a.originalName || `Audio ${i + 1}`}
                      </p>
                    )}
                    <audio controls className="w-full rounded-lg" src={a.src}>
              ...
              <p className="text-sm text-muted-foreground italic">
                {t("resourceDetail.audioComingSoon")}
              </p>
            )}
          </div>
        </ScrollReveal>
              ...
              <div className="space-y-3">
                {relatedBooks.map((book) => (
                  ...
                  >
                    ...
                    <div>
                      <Badge className={`mb-1 ${GRADE_BADGE[book.grade]}`}>
                        {t(`grades.${book.grade}`)}
                      </Badge>
