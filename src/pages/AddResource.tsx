...
export default function AddResource() {
  ...
  return (
    <Layout>
      <SEOHead title="Add a Resource — English With Hinda" description="Create a new resource entry for the Resources page." />
      ...
      <section className="container py-12 max-w-2xl">
        <ScrollReveal>
          ...
          <p className="text-muted-foreground mb-8">
            {editingId ? "Update this resource entry." : "Create a new resource entry for the Resources page (stored in this browser only)."}
          </p>
        </ScrollReveal>
        ...
        {success && (
          <div className="flex items-center gap-3 rounded-xl border border-primary/20 bg-primary/5 p-4 mb-6">
            ...
            <p className="text-sm font-medium">Resource {editingId ? "updated" : "saved"} successfully! It will appear on the Resources page.</p>
          </div>
        )}
        ...
        <ScrollReveal>
          <form className="space-y-5 rounded-xl border bg-card p-6 sm:p-8 shadow-sm" onSubmit={handleSubmit}>
            ...
            <div>
              ...
              <Input id="title" value={form.title} onChange={(e) => set("title", e.target.value)} placeholder="e.g., Grammar Sheet — Simple Present" className="mt-1.5" />
            ...
            <div className="grid gap-5 sm:grid-cols-2">
              <div>
                ...
                <Select value={form.level} onValueChange={(v) => set("level", v)}>
                  ...
                  <SelectContent>
                    {LEVELS.map((l) => <SelectItem key={l} value={l}>{l} Year</SelectItem>)}
                  </SelectContent>
                </Select>
            ...
            <div>
              ...
              <Select value={form.skill} onValueChange={(v) => set("skill", v)}>
                <SelectTrigger className="mt-1.5"><SelectValue placeholder="Select skill…" /></SelectTrigger>
                <SelectContent>
            ...
            <div>
              ...
              <Textarea id="desc" value={form.description} onChange={(e) => set("description", e.target.value)} placeholder="What is inside and how to use it…" rows={3} className="mt-1.5" />
            ...
            <div>
              ...
              <Input id="url" value={form.url} onChange={(e) => set("url", e.target.value)} placeholder="https://drive.google.com/..." className="mt-1.5" />
            ...
            <div className="grid gap-5 sm:grid-cols-2">
              <div>
                ...
                <Input id="duration" value={form.duration} onChange={(e) => set("duration", e.target.value)} placeholder="e.g., 5 min" className="mt-1.5" />
              </div>
              <div>
                <Label htmlFor="pages">Pages (optional)</Label>
                <Input id="pages" type="number" value={form.pages} onChange={(e) => set("pages", e.target.value)} placeholder="e.g., 2" className="mt-1.5" />
              </div>
            </div>
            ...
            <div>
              ...
              <Input id="tags" value={form.tags} onChange={(e) => set("tags", e.target.value)} placeholder="e.g., revision, exam, basics" className="mt-1.5" />
            </div>
        ...
        {showPreview && form.title && (
          <ScrollReveal>
            <div className="mt-8">
              <h3 className="font-serif text-lg font-semibold mb-3">Card Preview</h3>
              <div className="rounded-xl border bg-card p-5 shadow-sm max-w-sm">
                <div className="flex items-center gap-2 mb-3 flex-wrap">
                  {form.level && <Badge className={LEVEL_BADGE[form.level] || "bg-muted"}>{form.level} Year</Badge>}
        ...
        <ScrollReveal>
          <div className="mt-12">
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-serif text-xl font-bold">Manage Resources ({resources.length})</h2>
              {resources.length > 0 && (
            ...
            <p className="text-xs text-muted-foreground mb-4">These resources are stored in your browser only.</p>

            {resources.length === 0 ? (
              <div className="rounded-xl border bg-muted/50 p-8 text-center">
                <AlertCircle className="mx-auto h-8 w-8 text-muted-foreground mb-2" />
                <p className="text-sm text-muted-foreground">No resources stored yet. Add one above!</p>
              </div>
            ) : (
                  ...
                  <div key={r.id} className="flex items-center gap-3 rounded-xl border bg-card p-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        <Badge className={`text-[10px] ${LEVEL_BADGE[r.level] || "bg-muted"}`}>{r.level} Year</Badge>
