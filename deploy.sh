...
#############################################
APP_DIR="${APP_DIR:-/var/www/learnenglish/front}"
BRANCH="${BRANCH:-production}"
REPO_URL="${REPO_URL:-}"
BUILD_DIR="${BUILD_DIR:-dist}"

ENV_FILE="$APP_DIR/.env"
ENV_PAYLOAD_PATH="${ENV_PAYLOAD_PATH:-/tmp/learnenglish.env}"

PM2_APP="${PM2_APP:-front}"
...
if [[ ! -d "$APP_DIR/.git" || ! -f "$APP_DIR/package.json" ]]; then
  ...
  TMP_CLONE="/tmp/learnenglish_clone_$$"
